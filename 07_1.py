...
predition = tf.nn.softmax(tf.matmul(L2_drop, W3) + b3)

# 二次代价函数
# loss = tf.reduce_mean(tf.square(y - predition))
loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=predition))
...
with tf.Session() as sess:
    ...
    for epoch in range(51):  # 整体循环21次训练
        ...
        print("Iter " + str(epoch) + " ,Test Accuracy:" + str(acc) + " ,learing_rate:" + str(learning_rate))
        print('--------------------------------------------------------')
